...
class PipelineFailDao {
    ...
    fun queryPipelineFailErrorTypeInfo(
        ...
        with(TPipelineFailSummaryData.T_PIPELINE_FAIL_SUMMARY_DATA) {
            ...
            val conditions = getConditions(queryReqDO.projectId, queryReqDO.queryReq, tProjectPipelineLabelInfo)
    ...
    private fun TPipelineFailDetailData.getConditions(
        ...
        if (!baseQueryReq.pipelineLabelIds.isNullOrEmpty()) {
            conditions.add(pipelineLabelInfo.LABEL_ID.`in`(baseQueryReq.pipelineLabelIds))
        }
        val formatter  = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")
        val startTimeDateTime = LocalDateTime.parse(baseQueryReq.startTime, formatter)
        val endTimeDateTime = LocalDateTime.parse(baseQueryReq.endTime, formatter)
