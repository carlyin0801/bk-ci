    ...
    fun queryPipelineSummaryInfo(
        ...
        with(TProjectThirdPlatformData.T_PROJECT_THIRD_PLATFORM_DATA) {
            ...
            return dslContext.select(
                sum(REPO_CODECC_AVG_SCORE).`as`("REPO_CODECC_AVG_SCORE"),
                sum(RESOLVED_DEFECT_NUM).`as`("RESOLVED_DEFECT_NUM"),

                sum(QUALITY_PIPELINE_INTERCEPTION_NUM).`as`("QUALITY_PIPELINE_INTERCEPTION_NUM"),
                sum(QUALITY_PIPELINE_EXECUTE_NUM).`as`("QUALITY_PIPELINE_EXECUTE_NUM"),
                sum(TURBO_SAVE_TIME).`as`("TURBO_SAVE_TIME")
            ).from(this)
